...
Tất cả dữ liệu (config, tiến độ, session) lưu trong app_dir — KHÔNG bị mất khi
đóng app, kể cả khi chạy bằng file exe (PyInstaller). Xem paths.py.
...
def _resolve_session_dir(name: str, raw: str) -> str:
    """Nếu raw trống → app_dir/session/<name>; nếu relative → ghép app_dir."""
    base = app_dir()
    if not raw:
        return os.path.join(base, "session", name)
    if os.path.isabs(raw):
        return raw
    return os.path.join(base, raw)


def _resolve_path(raw: str, default_if_empty: str) -> str:
    """Resolve 1 đường dẫn: trống → default; relative → ghép app_dir."""
    base = app_dir()
    if not raw:
...
def load_multi_config(path: str) -> MultiConfig:
    ...
    for a in accts_raw:
        ...
        accounts.append(Account(
            name=name,
            session_dir=_resolve_session_dir(name, a.get("session_dir", "") or ""),
    ...
    # progress_dir: trống → app_dir/progress_multi.
    progress_dir = data.get("progress_dir", "") or ""
    if not progress_dir:
        progress_dir = os.path.join(app_dir(), "progress_multi")
    elif not os.path.isabs(progress_dir):
        progress_dir = os.path.join(app_dir(), progress_dir)

    # image: relative → ghép app_dir (để chạy exe cũng thấy ảnh).
    image = data.get("image", "") or ""
    if image and not os.path.isabs(image) and not os.path.exists(image):
    ...
    return MultiConfig(
        ...
        palette_path=_resolve_path(data.get("palette", "") or "", ""),
        ...
        start_delay_seconds=float(data.get("start_delay_seconds", 5.0)),
...
def run_worker(
    ...
    try:
        ...
        if acc.x_start is None or acc.x_end is None:
            ...
            log("❌ Thiếu x_start/x_end. Dùng --auto-split hoặc điền tay trong config.")
            return
        ...
        try:
            ...
            # 4) Build Config giả để dom_painter dùng.
            ...
            # 6) Đếm ngược trước khi vẽ (để bạn kịp theo dõi).
            if mc.start_delay_seconds > 0:
                state.status = "wait"
                for i in range(int(mc.start_delay_seconds), 0, -1):
                    state.last_msg = f"bắt đầu sau {i}s"
                    log(f"⏱ Bắt đầu vẽ sau {i}s...")
                    time.sleep(1)
...
def _build_cfg(mc: MultiConfig, acc: Account) -> Config:
    ...
    cfg.pixel_offset_x = mc.offset_x
    cfg.pixel_offset_y = mc.offset_y
    cfg.pixel_start_delay_seconds = mc.start_delay_seconds
    return cfg
...
def paint_all(mc: MultiConfig, reset: bool = False, only: Optional[str] = None,
    ...
    if not mc.image or not os.path.exists(mc.image):
        log(f"❌ Không thấy ảnh: {mc.image!r}. Điền 'image' trong accounts.yaml.")
        return 1
...
""",
    ...
    sub.add_parser("gui", help="Mở giao diện đồ họa (Tkinter).")

    args = parser.parse_args()

    if args.cmd == "gui":
        # GUI tự load accounts.yaml nên không cần parse config ở đây.
        from multi_gui import main as gui_main
        gui_main()
        return

    # Resolve config path relative to app_dir nếu không phải tuyệt đối.
    cfg_path = args.config
    if not os.path.isabs(cfg_path) and not os.path.exists(cfg_path):
        cfg_path = os.path.join(app_dir(), cfg_path)

    mc = load_multi_config(cfg_path)
